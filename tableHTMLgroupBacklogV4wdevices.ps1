...
foreach ($day in $dailyBreakdownUsers) {
    $dayDate = $day.Name
    $dayLogs = $day.Group
...
foreach ($day in $dailyBreakdownDevices) {
    $dayDate = $day.Name
    $dayLogs = $day.Group
...
foreach ($day in $dailyBreakdownGroups) {
    $dayDate = $day.Name
    $dayLogs = $day.Group
...
foreach ($day in $dailyBreakdownAdminUnits) {
    $dayDate = $day.Name
    $dayLogs = $day.Group
...
new Chart(document.getElementById('userActivityChart'), config);
</script>
"@
...
const config = {
  ...
  options: {
    ...
    plugins: {
      legend: { position: 'top' },
      title: { display: true, text: 'Device Management Activity' }
    }
  }
};

new Chart(document.getElementById('deviceActivityChart'), config);
</script>
"@
...
new Chart(document.getElementById('adminUnitChart'), config);
</script>
"@
