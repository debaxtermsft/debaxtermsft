...
#>
#GCC/GCCHigh
connect-mggraph -scopes "directory.read.all, auditlog.read.all, user.read.all" -TenantId <tenantid> -Environment USGov
#GCC DOD
#connect-mggraph -scopes "directory.read.all, auditlog.read.all, user.read.all" -TenantId <tenantid> -Environment USGovDoD

#by UPN
$ApiUrl = "https://graph.microsoft.us/v1.0/users?`$filter=startswith(userprincipalname,'derrick.baxter@saviors18twd.onmicrosoft.com')&`$select=displayName,userprincipalname,signInActivity"
#by date
