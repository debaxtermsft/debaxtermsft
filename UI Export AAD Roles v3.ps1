...
function find-aadroleinfo($type)
{
        ...
        foreach ($item in $aadrole)
        {
           if ($type -eq "All")
           {
               $aadrolemember = get-AzureADDirectoryRolemember -objectid $item.ObjectId | select displayname, objecttype, objectid, userprincipalname | sort-object objecttype, displayname
           }
           else
           {
               $aadrolemember = get-AzureADDirectoryRolemember -objectid $item.ObjectId | Where-Object {$_.ObjectType -eq $type}  | select displayname, objecttype, objectid, userprincipalname | sort-object displayname
           }
                       ...
                       # $aadroleassignment
                        ...
                        else
                        {
                              ...
                              $members += New-Object Object |
                                        ...
                                        Add-Member -NotePropertyName AADID -NotePropertyValue $aadroleassignment.Id -PassThru |
                                        Add-Member -NotePropertyName AADDirectoryScope -NotePropertyValue $aadroleassignment.DirectoryScopeId -PassThru
