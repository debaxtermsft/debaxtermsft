...
# V1.1
# updated 6/27/22
# fixed subscription looping, added sorting of subscription name for management group(s)
...
do
    {
        # select of 1 file or 1 file per sub or mgmt group/sub in the mgmt group(s)
        ...
        if ($pimfilterselection -eq "Cancel"){exit}

        switch -regex ($pimfilterselection) 
        {
            "Cancel" 
                {
                    Write-Host "Exiting" 
        ...
        if($pimfilterselection -ne "Subscription")
        {
            ...
            foreach($subselect in $exid)
            {
                ...
                if ($mgmtsubscription -ne $null)
                {
                    ...
                    {
                        $subname = Get-AzSubscription -SubscriptionName $mgsubitem.DisplayName
                        $subid1 += New-Object Object |
                                                Add-Member -NotePropertyName "SubscriptionName"     -NotePropertyValue $subname.Name -PassThru |
                                                Add-Member -NotePropertyName "SubscriptionID"            -NotePropertyValue $subname.id -PassThru 
                        #$subid += $subname.Id +" : " + $subname.Name 

                    }
                    $subid2 = $subid1 | Sort-Object subscriptionname
                    $subid =@()
                    foreach($subitemselecteditem2 in $subid2)
                    {
                        $subid += $subitemselecteditem2.subscriptionid +" : " + $subitemselecteditem2.subscriptionname
                    }
                    ...
                    if ($mgmtsubscriptionselection -eq "Cancal"){exit}
        ...
        foreach ($selecteditem in $subscriptionselection)
            {
                ...
                if ($RBACAssignments1.count -eq 0){ write-host "No PIM Assignments" $selecteditem}
                else
                {
                    foreach ($rbacitem1 in $RBACAssignments1)
                        {
                            ...
                            if ($RBACPRA1.count -gt 1000) {$setsleep -eq $true}
                            foreach ($praitem1 in $RBACPRA1)
                            {
                                ...
                                if ($setsleep -eq $true ) {Start-Sleep -Seconds 1 $setsleep = $false}
                            }
                        ...
                        { 
                            ...
                            $filename1  = "PIM Assignment Single File Export " + $pimfilterselection +" "+ $rbacitem1.DisplayName+" "+ $tdy +".csv"           
        ...
        { 
            ...
            $tdy        = get-date -Format "MM-dd-yyyy hh.mm.ss"
            $filename1  = "PIM Assignment Export All in One Export " + $pimfilterselection + " " + $tdy +".csv"           
            $OutputFile = "$outputdirectory"+"\"+"$filename1"
            $rbacroles | export-csv -Path $outputfile -force -Encoding UTF8 -NoTypeInformation
            $rbacroles =@()
        }
