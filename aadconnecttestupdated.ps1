    ...
    foreach ($url in $RequiredResources)
    {
        ...
        foreach ($ip4 in $ResourceAddresses)
        {
            try
            {
                ...
                switch ($Result80.TcpTestSucceeded)
                {
                    true { Write-Log -LogFile $Logfile -LogLevel SUCCESS -Message "TCP connection to $($url) [$($ip4)]:80 successful." -ConsoleOutput }
                    false { Write-Log -LogFile $Logfile -LogLevel ERROR -Message "TCP connection to $($url) [$($ip4)]:80 failed." -ConsoleOutput;  }
                }
