...
DN :  CN=R2 D3,OU=users,OU=Sales,OU=corp,DC=synclab,DC=local
 ...
 .\EntraIDConnectPermissionsInheritanceChecker.ps1 -dn "OU=corp,DC=synclab,DC=local" -PermissionsCheck 'Entra Connect Service Account' -outputfile output.csv -adcserviceprincipal MSOL_xxxxyyyyzzzz
 
 Searching on a user to see if they have the correct permissions for Inheritance Disabled
.\EntraIDConnectPermissionsInheritanceChecker.ps1 -dn "OU=corp,DC=synclab,DC=local" -PermissionsCheck Inheritance -outputfile output.csv -adcserviceprincipal MSOL_xxxxyyyyzzzz
Unsure of the MSOL full account name - just use MSOL or NOTHING
.\EntraIDConnectPermissionsInheritanceChecker.ps1 -dn "OU=corp,DC=synclab,DC=local" -PermissionsCheck Inheritance -outputfile output.csv -adcserviceprincipal MSOL
.\EntraIDConnectPermissionsInheritanceChecker.ps1 -dn "OU=corp,DC=synclab,DC=local" -PermissionsCheck Inheritance -outputfile output.csv 

OU Inheritance Check
Entire Forest
.\EntraIDConnectPermissionsInheritanceChecker.ps1 -PermissionsCheck Inheritance -outputfile output1424 -OUorUser OU
or
.\EntraIDConnectPermissionsInheritanceChecker.ps1 -dn "DC=synclab,DC=local" -PermissionsCheck Inheritance -outputfile output1424 -OUorUser OU
Specific OU (sublevels)
.\EntraIDConnectPermissionsInheritanceChecker.ps1 -dn "OU=corp,DC=synclab,DC=local" -PermissionsCheck Inheritance -outputfile output1424 -OUorUser OU
...
else # "OU" selected
{
    ...
    if($PermissionsCheck -eq "Inheritance")
    {
        $orgunit = Get-ADOrganizationalUnit -filter *  -Properties nTSecurityDescriptor -SearchBase $dn
        foreach ($item in $orgunit) 
        {
            
            $orgsec = $item.ntsecuritydescriptor.AreAccessRulesProtected
            $exportinheritancecheck += New-Object object |
                Add-Member -NotePropertyName DN -NotePropertyValue $item.DistinguishedName -PassThru |
                Add-Member -NotePropertyName InheritanceDisabled -NotePropertyValue $orgsec -PassThru
        }
            $exportinheritancecheck
        $filename2 = $OUorUser+$PermissionsCheck+"_Check_"+ $outputfile + "_"+$tdy +".csv"
        $exportfile = $exportinheritancecheck | Where-Object{$_.inheritancedisabled -eq "True"} 
    ...
    else # checking MSOL permissions"
    {
